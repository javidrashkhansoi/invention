...
const script_form = document.forms["form"];

if (script_form) {
  /** @type {HTMLInputElement} */
  const nameInput = script_form["form-name"];
  const nameSpan = document.getElementById("name");
  const invention = document.getElementById("invention");
  /** @type {RadioNodeList} */
  const withes = script_form["form-with"];
  const withSpan = document.getElementById("with");
  /** @type {RadioNodeList} */
  const genders = script_form["form-gender"];
  const genderSpan = document.getElementById("gender");

  if (nameInput && nameSpan) {
    nameInput.addEventListener("input", () => {
      nameSpan.innerText = nameInput.value ?
        `${nameInput.value[0].toLocaleUpperCase("az")}${nameInput.value.substring(1)}` :
        "Ad";
    });
    ...
    script_form.addEventListener("submit", event => {
      ...
      const imageName = nameSpan.innerText;

      toJpeg(invention, { quality: 1, canvasWidth: 2444, canvasHeight: 3168 })
        .then(dataUrl => {
  ...
  if (withes.length && withSpan) {
    withes.forEach(radio => {
      radio.addEventListener("change", () => {
        if (withes.value === "Tək") {
          withSpan.innerText = "";
        } else {
          withSpan.innerText = withes.value === "Ailəvi" ?
            "və ailənizi" :
            "və xanımınızı";
        }
      });
  ...
  if (genders.length && genderSpan) {
    genders.forEach(radio => {
      radio.addEventListener("change", () => {
        genderSpan.innerText = genders.value === "Bəy" ?
          "bəy" :
          "xanım";
      });
    });
